...
import { Home, Compass, Search, Bookmark, ShieldAlert, Settings, Zap, Home as HomeIcon } from 'lucide-react';
...
const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [settings, setSettings] = useState<AppSettings>(() => {
    const saved = localStorage.getItem('mangaverse_settings_v3');
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('mangaverse_settings_v3', JSON.stringify(settings));
  }, [settings]);
  ...
  return (
    <nav className="hidden lg:flex fixed left-0 top-0 bottom-0 w-24 flex-col items-center py-8 bg-zinc-950 border-r border-zinc-800/50 z-50">
      ...
      <div className="flex flex-col gap-6 flex-1">
        ...
        <NavLink to="/mature" icon={ShieldAlert} id="mature" />
      </div>
...
const MobileNav = () => {
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-zinc-950/90 header-blur border-t border-zinc-800/50 px-2 py-2 flex items-center justify-around shadow-2xl">
      ...
      <NavLink to="/mature" icon={ShieldAlert} label="Mature" />
    </nav>
  );
};
      ...
      <Router>
        <Layout>
          <Routes>
            ...
            <Route path="/mature" element={<MaturePage />} />
          </Routes>
        </Layout>
      </Router>
    </AppProvider>
