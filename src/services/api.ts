...
export const EXCLUDED_TAGS = ['d8a9547b-9159-48d4-92f0-9d0c357042a8', 'ddefd648-5140-4e5f-ba18-4eca4071d19b']; // Loli, Shota
...
export async function apiFetch(endpoint: string, params: any = {}) {
  ...
  // Apply Global Policy: Enforce all content ratings and restricted language (English)
  // unless explicitly overridden for special pages.
  ...
  try {
    ...
    if (err.name === 'TypeError' || err.message.includes('Failed to fetch')) {
      throw new Error("Connection Blocked (CORS). Please check Settings.");
    }
    throw err;
  }
}
...
export function getTags(manga: Manga) {
  return manga.attributes.tags.map(t => t.attributes.name.en);
}
